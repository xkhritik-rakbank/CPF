...
public class CPFMain implements Runnable {
		...
		try 
		{
			...
			if(Maincode.equalsIgnoreCase("0"))
			{
				...
				for (; objWorkList.hasMoreElements(true); objWorkList.skip(true))
				{
					...
					criteria=objWorkList.getVal("CommCriteria");
					frequency=objWorkList.getVal("CommFrequencyInDays");
					String nextCommFreq=objWorkList.getVal("nextCommFreq");
					String CommQuery=objWorkList.getVal("CommQuery");
					CommQuery=CommQuery.replace("$Frequency$", frequency);
					CommQuery=CommQuery.replace("$nextCommFreq$", nextCommFreq);
					getWorkItemListToProcess(CommQuery);
	...
	protected static void getWorkItemListToProcess(String CommQuery)
	{
		//List<String> WIList = new ArrayList<String>();
		...
		else if(processName.equalsIgnoreCase("BAIS"))
		{
			switch(stage)
			{
				...
				{
					query="SELECT Q.processinstanceid,Q.IntroductionDateTime,Q.activityid,Q.processdefid,(dbo.CPF_GetCutOffTime_Function(ext.WI_NAME,q.IntroductionDateTime,'"+nextCommFreq+"')) as NextCommDate FROM QUEUEVIEW AS Q with(nolock) "+
							" JOIN RB_BAIS_EXTTABLE AS EXT with(nolock) ON Q.processinstanceid = EXT.WI_NAME "+
							" WHERE Q.ProcessName ='BAIS' "+
							" AND ( "+
								" (Q.activityname ='Sys_CPF_Response' "+ 
									" AND (EXT.CPFStage1Status IS NULL OR CPFStage1Status = '') "+ 
									" AND (EXT.CPFDeclineStatus ='None') "+
		...
		try {
			...
			XMLParser objXMLParser = new XMLParser();
			String Maincode="";
			CPFLog.CPFLogger.info("Query to fetch WIList = "+CommQuery);
			String InputXML = CommonMethods.apSelectWithColumnNames(CommQuery, cabinetName, sessionID);
		...
		{ 
			...
			if(result!=null && result.size()>0)
			{
				...
				if(IsActiveMail.equals("Y")||IsActiveEnglish.equals("Y")||isActiveSMSArabic.equals("Y"))
				{
					...
					widata=getWIDataToCommunicate(WI_NAME,MailTemplate,SMSEnglishTemplate,SMSArabicTemplate,MailPlaceHolders,tempType,mailSubject,mailSubPlaceHolder,ccMail);
					
					if(IsActiveMail.equals("Y"))
					{
						if(widata!=null && widata.size()!=0)
						CPFCommunication.sendMail(fromMail, widata.get(0),widata.get(5), cabinetName, sessionID,widata.get(2), widata.get(3), ProcessDefID, activityId,lstage,WI_NAME,wsName,processName,dataSource,tempType);
					}
					if(isActiveSMSArabic.equals("Y")||IsActiveEnglish.equals("Y"))
					{
						...
						if(widata!=null && widata.size()!=0) {
							...
							{
								CPFCommunication.sendSMS(widata.get(1), widata.get(6), cabinetName, sessionID, ProcessDefID,wsName,lstage, WI_NAME, processName,dataSource,tempType);
							}
						}
					}
				}
	...
	protected static List<String> getWIDataToCommunicate(String WI_NAME,String mailTemp,String SMSEnglishTemplate, String SMSArabicTemplate,String mailPlaceHolder,String tempType,String mailSubject,String mailSubjectPlaceholder, String ccMail)//yet to be completed
	{
		List<String> widata = new ArrayList<String>();
		String query="";
		String mainCode="";
		...
		try
		{
			switch(processName)
			{
				case "AO":
				{
					query="SELECT E.eMail,concat(E.Mobile_Country_Code,'',E.Mobile_Number) AS MobNo,E.Retail_or_Corporate,W.customer_name1,(SELECT SOLENAME FROM RB_BRANCH_MASTER with(nolock) WHERE right(SOLEID,3)=right(W.Initiating_SOL_ID,3)) AS BRANCH " + 
			        ...
			        if(mainCode.equalsIgnoreCase("0"))
			        {
						...
						for(String s:pHArr)
						{
							...
							switch(s)
							{
								case "$WINAME$":
								{
									...
									SMSArabicTemplate=SMSArabicTemplate.replace(s,lURN);
									break;
								}
								case "$Product$":
								{
						...
						widata.add(ccMail);
						widata.add(SMSArabicTemplate);
					}
			        break;
				}
				case "BAIS":
				{
					query="SELECT EMAILID,concat(MOBILECNTRYCODE,'',MOBILENO) AS MobNo, "
							+ "isnull((select PersonalName+' '+FamilyName from PDBUser with (nolock) where username=rocode),rocode) as roname, "
							+ "isnull((select MailId from PDBUser with (nolock) where username=rocode),'') as romailid, "
							+ "isnull((select MailId from PDBUser with (nolock) where username=rmcode),'') as rmmailid, "
							+ "custname,(SELECT SOLENAME FROM RB_BRANCH_MASTER with(nolock) WHERE right(SOLEID,3)=right(Sol_Id,3)) AS BRANCH, TLNumber " + 
							" FROM RB_BAIS_EXTTABLE  with(nolock) " + 
			        ...
			        if(mainCode.equalsIgnoreCase("0"))
			        {
			        	String eMail=CommonMethods.getTagValues(sOutputXML,"EMAILID");
						if(eMail!=null && !"".equalsIgnoreCase(eMail))
							widata.add(eMail);
						...
						if(MobNo!=null && !"".equalsIgnoreCase(MobNo))
							widata.add(MobNo);
						String roname=CommonMethods.getTagValues(sOutputXML,"roname");
						String romailid=CommonMethods.getTagValues(sOutputXML,"romailid");
						String rmmailid=CommonMethods.getTagValues(sOutputXML,"rmmailid");
						if(!"".equalsIgnoreCase(ccMail.trim()))
						{
							if(!"".equalsIgnoreCase(romailid.trim()))
								ccMail=ccMail+","+romailid;
							if(!"".equalsIgnoreCase(rmmailid.trim()))
								ccMail=ccMail+","+rmmailid;
						}
						else
						{
							if(!"".equalsIgnoreCase(romailid.trim()))
						...
						String custName=CommonMethods.getTagValues(sOutputXML,"custname");
						String Branch=CommonMethods.getTagValues(sOutputXML,"BRANCH");
						lURN=getURN(WI_NAME);
						...
						String TLNumber=CommonMethods.getTagValues(sOutputXML,"TLNumber").trim();
						if(TLNumber.length() > 3)
						{
							TLNumber = maskStartingChar(TLNumber, 0, TLNumber.length() - 3, "X"); // Jira PCBS-41 masking TL Number leaving last 3 chars clear text
						}
						...
						for(String s:pHArr)
						{
							...
							switch(s)
							{
								...
								case "$Product$":
								{
									mailTemp = mailTemp.replace(s, "Corporate");
									break;
								}
								case "$nextCommDate$":
								{
								...
								case "$SalesAgentName$":
								{
									mailTemp = mailTemp.replace(s, roname); // need to check data
									break;
								}
								case "$SalesAgentContact$":
								{
								...
								case "$TradeLicense$":
								{
									mailTemp = mailTemp.replace(s, TLNumber);
									SMSEnglishTemplate=SMSEnglishTemplate.replace(s,TLNumber);
									SMSArabicTemplate=SMSArabicTemplate.replace(s,TLNumber);
									break;
								}
								case "$RejectReason$":
								{
	...
	protected static String getCorrectTemplate(String WI_NAME,List<String> templateType)//yet to be completed
	{
		...
		for(int i=0;i<templateType.size();i++)
		{
			...
			if("AO".equalsIgnoreCase(processName))
			{
				...
				else if("Reject".equalsIgnoreCase(wsName))
				{
					...
					if(ChecklistData!=null && !"".equalsIgnoreCase(ChecklistData))
					{
						...
						if(ChecklistData.contains("#"))
						{
							String data = "";
							String ChecklistDataTmp [] = ChecklistData.split("#");
							for(int k=0;k<ChecklistDataTmp.length;k++)
							{
								if(data.equalsIgnoreCase(""))
									data = "'"+ChecklistDataTmp[k].substring(0, 3)+"'";
								else
									data = data + ",'"+ChecklistDataTmp[k].substring(0, 3)+"'";
							}
							ChecklistData = data;
						}
						else
							ChecklistData = "'"+ChecklistData.substring(0,3)+"'";
			...
			{
				if("Reject".equalsIgnoreCase(wsName))
				{

					String query ="SELECT TOP(1) wsname,RejectReasons FROM USR_0_BAIS_WIHISTORY with(nolock) WHERE winame='"+WI_NAME+"' AND RejectReasons IS NOT NULL AND CAST(RejectReasons AS NVARCHAR) !='' ORDER BY actiondatetime DESC";
					//SELECT TOP(1) wsname , ChecklistData FROM usr_0_ao_wihistory WHERE winame='AO-0000000787-Process'  AND ChecklistData IS NOT NULL AND CAST(ChecklistData AS NVARCHAR) !='' ORDER BY actiondatetime DESC
					...
					String workstep=CommonMethods.getTagValues(sOutputXML,"wsname");
					String ChecklistData=CommonMethods.getTagValues(sOutputXML,"RejectReasons");
					if(ChecklistData!=null && !"".equalsIgnoreCase(ChecklistData))
					{
						ChecklistData=ChecklistData.replace("#", "','");
						//ChecklistData=ChecklistData.replace(":", "','");
	...
	public static String getRejectCategory(String workstep,String ChecklistData)
	{
		...
		try {
			...
			{
			case "AO":
				Query = "SELECT DISTINCT CPFRejectCategory FROM USR_0_AO_Error_Desc_Master with(nolock) WHERE Item_Code IN("+ChecklistData+")";
				break;
			case "BAIS":
				Query = "SELECT DISTINCT CPFRejectCategory FROM USR_0_BAIS_ERROR_DESC_MASTER with(nolock) WHERE Item_Code IN("+ChecklistData+")";
				break;
			}
			//Query = "SELECT DISTINCT CPFRejectCategory FROM USR_0_AO_Error_Desc_Master with(nolock) WHERE Item_Code IN("+ChecklistData+") AND WSName='"+workstep+"'";
			...
			String OutputXML=WFNGExecute(InputXML, jtsIP, jtsPort, 0 , CPFLog.CPFLogger);
			CPFLog.CPFLogger.info("Getting RejectCategory from Reject Reason Master = "+OutputXML);
	
			xmlParserData.setXmlString(OutputXML);
			Maincode=xmlParserData.getVal("MainCode");
			CPFLog.CPFLogger.info("Getting data from  Reject Reason Master table Maincode = "+Maincode);
			if(Maincode.equalsIgnoreCase("0"))
			{
				objWorkList = xmlParserData.createList("Records", "Record");
				CPFLog.CPFLogger.info("Getting data from Queueview Maincode = "+objWorkList);
				...
				if(rc!=null)
				{
					...
					{
						if("Policy".equalsIgnoreCase(rc.get(i))) // incase of multiple giving priority to Policy
						{
							RejectCategory="Policy";
							break;
						}
						else if("Compliance".equalsIgnoreCase(rc.get(i)))
						{
							RejectCategory="Compliance";
						}
					}
				}
			}
	...
	public static String getRejectReason(String WI_NAME,String tempType)
	{
		...
		{
		case "AO":
			query1 ="SELECT TOP(1) wsname,ChecklistData FROM usr_0_ao_wihistory with(nolock) WHERE winame='"+WI_NAME+"' AND ChecklistData IS NOT NULL AND CAST(ChecklistData AS NVARCHAR) !='' ORDER BY actiondatetime DESC";
			if("Pending with Customer".equalsIgnoreCase(tempType))
				query2 ="SELECT DISTINCT Item_Desc FROM USR_0_AO_Error_Desc_Master with(nolock) WHERE Item_Code IN(ChecklistData) AND PendingCommunication='Customer'";
			else if("Decline due to policy".equalsIgnoreCase(tempType))
				query2 ="SELECT DISTINCT Item_Desc FROM USR_0_AO_Error_Desc_Master with(nolock) WHERE Item_Code IN(ChecklistData) AND CPFRejectCategory='Policy'";
			
			break;
		case "BAIS":
			query1 ="SELECT TOP(1) wsname,RejectReasons as ChecklistData FROM USR_0_BAIS_WIHISTORY with(nolock) WHERE winame='"+WI_NAME+"' AND RejectReasons IS NOT NULL AND CAST(RejectReasons AS NVARCHAR) !='' ORDER BY actiondatetime DESC";
			if("Pending with Customer".equalsIgnoreCase(tempType))
				query2 ="SELECT DISTINCT Item_Desc FROM USR_0_BAIS_ERROR_DESC_MASTER with(nolock) WHERE Item_Code IN(ChecklistData) AND PendingCommunication='Customer'";
			else if("Decline due to policy".equalsIgnoreCase(tempType))
				query2 ="SELECT DISTINCT Item_Desc FROM USR_0_BAIS_ERROR_DESC_MASTER with(nolock) WHERE Item_Code IN(ChecklistData) AND CPFRejectCategory='Policy'";
			
			break;
		}
		
		
		String sInputXML=CommonMethods.apSelectWithColumnNames(query1, cabinetName, sessionID);
		CPFLog.CPFLogger.debug("Input XML: "+sInputXML);
		String sOutputXML =  null;
		try
		{
		...
		if(ChecklistData!=null && !"".equalsIgnoreCase(ChecklistData))
		{
			...
			query2=query2.replace("ChecklistData", ChecklistData);
			...
			try{
				...
				if(Maincode.equalsIgnoreCase("0"))
				{
					...
					{
						if("Pending with Customer".equalsIgnoreCase(tempType))
							rejectReason+="<li>Additional information required.</li>";
						else if("Decline due to policy".equalsIgnoreCase(tempType))
							rejectReason+="<li>Not meeting bank’s internal policy</li>";
					}
					rejectReason+="</ul>";
				}
			}
		...
		else // when no reject reason are selected
		{
			rejectReason="<ul style = \"font-family:Verdana;font-size:11px;\">";
	...
	protected static String isAnyCondition(String WI_NAME)
    {
        ...
        switch(processName)
        {
	        ...
	        case "BAIS":
	        {
	        	query ="SELECT count(*) AS recCount FROM usr_0_BAIS_wihistory WITH(nolock) WHERE wsname = 'OPS_DataEntryChecker' AND decision = 'Approve' AND winame= '"+WI_NAME +"'";
	        	break;
	        }
        }
        String sInputXML=CommonMethods.apSelectWithColumnNames(query, cabinetName, sessionID);
	...
	public static String maskStartingChar(String str, int Start, int End, String maskChar)
	{
		StringBuilder sbMaskString = new StringBuilder(End);
		for(int i=0; i<End; i++)
		{
			sbMaskString.append(maskChar);
		}
		return str.substring(0, Start) + sbMaskString.toString() + str.substring(Start + End);
	}
}
